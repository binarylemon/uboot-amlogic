...
int test_sub_l2cache(void)
{
	...
	if(i<ARRAY_SIZE(L2_cache_pattern)){
		sprintf(systest_info_line, "%s:%d: l2cache: test fail: Error address=0x%x, pattern=0x%x\n",
													__FILE__, __LINE__, result, pattern);
		systest_log(systest_info_line, SYSTEST_INFO_L2);
		return -1;
	}
	else{
		sprintf(systest_info_line, "l2cache test pattern count=%d\n", ARRAY_SIZE(L2_cache_pattern));
		systest_log(systest_info_line, SYSTEST_INFO_L2);
		return 1;	
	}	
...
//==============================================================
int l2cache_systest(int argc, char *argv[])
{
	//int status;
	if(argc < 2){					
		
		if(test_sub_l2cache() < 0)
			systest_log("l2cache: test fail.\n", SYSTEST_INFO_L1);	
		else
			systest_log("l2cache: test pass.\n", SYSTEST_INFO_L1);			
	}
	else
		systest_log("l2cache: test fail: parameter number error.\n", SYSTEST_INFO_L1);		
