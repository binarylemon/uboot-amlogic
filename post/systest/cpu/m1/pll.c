...
unsigned    clk_util_msr( unsigned  clk_mux,unsigned time )
{
    ...
    unsigned long test = readl(P_MSR_CLK_REG2);
    unsigned test1 = test/64;    
    return (test1);
}
...
int test_pll_max(struct clk *pclk, unsigned clk_mux, struct test_info *max_info)
{
	...
	for(freq=pclk->middle; freq<=freq_max; freq+=freq_resolution){
		...
		if((test = clk_util_clk_msr(clk_mux) != 0)){			
			max_info->freq = freq;
			max_info->N = param.N;
			max_info->M = param.M;
			max_info->OD = param.OD;
...
int test_sys_pll(void)
{
	...
	test_pll_max(&clk_sys_pll, sys_pll_div3_clk_mux, &max_info );
	test_pll_min(&clk_sys_pll, sys_pll_div3_clk_mux, &min_info);
	...
	systest_log("sys pll test info: \n", SYSTEST_INFO_L2);
	systest_log("      |	Fin	|	N		M	|	OD	FOUT	| \n", SYSTEST_INFO_L2);
	sprintf(systest_info_line, "min:  |	%d	|	%d		%d	|	%d	%ld	|\n", CONFIG_CRYSTAL_MHZ, 
												min_info.N, min_info.M, min_info.OD, min_info.freq);
	systest_log(systest_info_line, SYSTEST_INFO_L2);
	sprintf(systest_info_line, "max:  |	%d	|	%d		%d	|	%d	%ld	|\n", CONFIG_CRYSTAL_MHZ, 
...
int test_other_pll(void)
{
	...
	systest_log("other pll test info: \n", SYSTEST_INFO_L2);
	systest_log("      |	Fin	|	N		M	|	OD	FOUT	| \n", SYSTEST_INFO_L2);
	sprintf(systest_info_line, "min:  |	%d	|	%d		%d	|	%d	%ld	|\n", CONFIG_CRYSTAL_MHZ, 
												min_info.N, min_info.M, min_info.OD, min_info.freq);
	systest_log(systest_info_line, SYSTEST_INFO_L2);
...
int test_demod_pll(void)
{
	...
	test_pll_max(&clk_demod_pll, demod_pll_clk400_mux, &max_info);
	test_pll_min(&clk_demod_pll, demod_pll_clk400_mux, &min_info);
	...
	systest_log("demod:  pll test info: \n", SYSTEST_INFO_L2);
	systest_log("      |	Fin	|	N		M	|	OD	FOUT	| \n", SYSTEST_INFO_L2);
	sprintf(systest_info_line, "min:  |	%d	|	%d		%d	|	%d	%ld	|\n", CONFIG_CRYSTAL_MHZ, 
												min_info.N, min_info.M, min_info.OD, min_info.freq);
	systest_log(systest_info_line, SYSTEST_INFO_L2);
...
int test_audio_pll(void)
{
	...
	systest_log("audio:  pll test info: \n", SYSTEST_INFO_L2);
	systest_log("      |	Fin	|	N		M	|	OD	FOUT	| \n", SYSTEST_INFO_L2);
	sprintf(systest_info_line, "min:  |	%d	|	%d		%d	|	%d	%ld	|\n", CONFIG_CRYSTAL_MHZ, 
												min_info.N, min_info.M, min_info.OD, min_info.freq);
	systest_log(systest_info_line, SYSTEST_INFO_L2);
...
int test_video_pll(void)
{
	...
	systest_log("video:  pll test info: \n", SYSTEST_INFO_L2);
	systest_log("      |	Fin	|	N		M	|	OD	FOUT	| \n", SYSTEST_INFO_L2);
	sprintf(systest_info_line, "min:  |	%d	|	%d		%d	|	%d	%ld	|\n", CONFIG_CRYSTAL_MHZ, 
												min_info.N, min_info.M, min_info.OD, min_info.freq);
	systest_log(systest_info_line, SYSTEST_INFO_L2);
...
//======================================================================================================
int pll_systest(int argc, char *argv[])
{
	int i,j;
	struct clk_lookup *ptest;
	int ret;
	...
	if(argc < 2){
		...
		for(i=0; i<clk_lookups_size; i++){
			if(lookups[i].pll_test() < 0){
				sprintf(systest_info_line, "pll: %s: test fail.\n", lookups[i].dev_id);				
				ret = -1;
			}
			else
					sprintf(systest_info_line, "pll: %s: test pass.\n", lookups[i].dev_id);				
			systest_log(systest_info_line, SYSTEST_INFO_L2);
		}
		
		if(ret < 0)
			systest_log("pll: test fail.\n", SYSTEST_INFO_L1)		;
		else
			systest_log("pll: test pass.\n", SYSTEST_INFO_L1);
		...
		for(i=1; i<argc; i++){
			ptest = NULL;
			for(j=0; j<clk_lookups_size; j++){
				if(strcmp((lookups[j].dev_id), argv[1]) == 0){
					ptest = (lookups+j);
					break;
				}
			}
			...
			if(ptest){			
				if((ptest->pll_test) && (ptest->pll_test() >= 0))
					sprintf(systest_info_line, "pll: %s: test pass.\n", argv[i]);														
				else{
						sprintf(systest_info_line, "pll: %s: test fail.\n", argv[i]);										
						ret = -1;
				...
				sprintf(systest_info_line, "pll: %s: not found the pll name. \n", argv[i]);
				ret = -1;
			}			
			systest_log(systest_info_line, SYSTEST_INFO_L2);	
		}
