...
static struct ddr_set __ddr_setting={
	...
	.t_pub_dtpr={
		...
		[1] =  (((CFG_DDR_MRD) << 0 ) |		//tMRD      //4 TCK     For DDR3 and LPDDR2, the value used for tMRD is 4 plus the value programmed in these bits, i.e. tMRDvalue for DDR3 (ranges from 4 to 7)
	...
	.t_ddr_pll_cntl= (1<<20)|(CFG_PLL_OD << 16)|(CFG_PLL_N<<9)|(CFG_PLL_M<<0),
	...
	.t_mmc_ddr_ctrl= (CONFIG_DDR_CHANNEL_SWITCH<<26)|  //bit 27:24.   ddr chanel selection.
						...
						(0<<12) |						//bit 12        ddr1 rank size.  0, 1, one rank.  2 : 2 ranks.
						(CONFIG_DDR1_ROW_SIZE<<10)|		//bit 11:10      ddr1 row size.  2'b01 : A0~A12.   2'b10 : A0~A13.  2'b11 : A0~A14.  2'b00 : A0~A15.
						(CONFIG_DDR1_COL_SIZE<<8) |		//bit 9:8      ddr1 col size.  2'b01 : A0~A8,    2'b10 : A0~A9.
						...
						(CONFIG_DDR_BANK_SET <<5) |		//bit 6:5.     ddr0 address map bank mode
														                    ...
														                    // 10 = address switch between 2 banks  bank[0] selection bits [8].
														                    // 11 = address switch between 4 banks  bank[1:0] selection bits [9:8].
						(0<<4) |						//bit 4        ddr0 rank size.  0, 1, one rank.  2 : 2 ranks.
						(CONFIG_DDR0_ROW_SIZE<<2) |		//bit 3:2      ddr0 row size.  2'b01 : A0~A12.   2'b10 : A0~A13.  2'b11 : A0~A14.  2'b00 : A0~A15.
						(CONFIG_DDR0_COL_SIZE<<0),		//bit 1:0      ddr0 col size.  2'b01 : A0~A8,    2'b10 : A0~A9.
...
={
	...
	.vid_pll_cntl = 0x5000023d,
	.vid2_pll_cntl = 0,
	.gp_pll_cntl = 0x40120232,
	.gp2_pll_cntl = 0x40120232,
