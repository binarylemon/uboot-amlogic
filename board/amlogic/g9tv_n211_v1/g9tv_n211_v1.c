...
u32 reboot_mode_suspend = reboot_mode;
	...
	//if(!strcmp(is_suspend, "true")){
	if ((reboot_mode_suspend != MESON_UBOOT_SUSPEND)) {
	//set low gpiow_0 for stb

		CLEAR_CBUS_REG_MASK(PREG_PAD_GPIO0_EN_N, 1 << 0);
		CLEAR_CBUS_REG_MASK(PREG_PAD_GPIO0_O, 1 << 0);
