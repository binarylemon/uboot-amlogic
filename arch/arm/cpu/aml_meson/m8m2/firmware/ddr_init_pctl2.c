...
#define DDR_PUB_PGSR0_DLL_LOCK	(1)
#define DDR_PUB_PGSR0_DDL_DONE	(2)
#define DDR_PUB_PGSR0_ZQ_DONE	(3)
#define DDR_PUB_PGSR0_PHY_INIT	(0)

#define DDR0_PUB_PGSR0_DDL_DONE	(2)
#define DDR0_PUB_PGSR0_DDL_DONE	(2)
#define DDR0_PUB_PGSR0_DDL_DONE	(2)
#define DDR0_PUB_PGSR0_DDL_DONE	(2)
#define DDR0_PUB_PGSR0_DDL_DONE	(2)
...
mmc_init_all:
	...
	unsigned int ddr_channel_0 = 1;//cfg_ddr_channel & 0x1;
	unsigned int ddr_channel_1 = 1;//(cfg_ddr_channel >> 1) & 0x1;
#if 0
	serial_puts("ddr_channel_0 : ");
	serial_put_dec(ddr_channel_0);
	serial_puts("\nddr_channel_1 : ");
	serial_put_dec(ddr_channel_1);
	serial_puts("\n");
#endif
...
pub_init_ddr1:
	...
	if(ddr_channel_0){
		while(!(readl(P_DDR0_PUB_PGSR0) >> DDR_PUB_PGSR0_DLL_LOCK) & 0x1)
			serial_puts("DDR0_PUB_PGSR0_DLL_LOCK while loop\n");
		while(!(readl(P_DDR0_PUB_PGSR0) >> DDR_PUB_PGSR0_DDL_DONE) & 0x1)
			serial_puts("DDR0_PUB_PGSR0_DDL_DONE while loop\n");
		while(!(readl(P_DDR0_PUB_PGSR0) >> DDR_PUB_PGSR0_ZQ_DONE) & 0x1)
			serial_puts("DDR0_PUB_PGSR0_ZQ_DONE while loop\n");
		while(!(readl(P_DDR0_PUB_PGSR0) >> DDR_PUB_PGSR0_PHY_INIT) & 0x1)
			serial_puts("DDR0_PUB_PGSR0_PHY_INIT while loop\n");
		while(!(readl(P_DDR0_PCTL_DFISTSTAT0) & 1)) {
			//DDR_INIT_WHILE_LOOP(loop_whi_count);
			serial_puts("P_DDR0_PCTL_DFISTSTAT0 while loop\n");
		}
		writel(1, P_DDR0_PCTL_POWCTL);
		while(!(readl(P_DDR0_PCTL_POWSTAT) & 1) ) {
			//DDR_INIT_WHILE_LOOP(loop_whi_count);
			serial_puts("P_DDR0_PCTL_POWSTAT while loop\n");
		}
	}

	if(ddr_channel_1){
		while(!(readl(P_DDR1_PUB_PGSR0) >> DDR_PUB_PGSR0_DLL_LOCK) & 0x1)
			serial_puts("DDR1_PUB_PGSR0_DLL_LOCK while loop\n");
		while(!(readl(P_DDR1_PUB_PGSR0) >> DDR_PUB_PGSR0_DDL_DONE) & 0x1)
			serial_puts("DDR1_PUB_PGSR0_DDL_DONE while loop\n");
		while(!(readl(P_DDR1_PUB_PGSR0) >> DDR_PUB_PGSR0_ZQ_DONE) & 0x1)
			serial_puts("DDR1_PUB_PGSR0_ZQ_DONE while loop\n");
		while(!(readl(P_DDR1_PUB_PGSR0) >> DDR_PUB_PGSR0_PHY_INIT) & 0x1)
			serial_puts("DDR1_PUB_PGSR0_PHY_INIT while loop\n");
		while(!(readl(P_DDR1_PCTL_DFISTSTAT0) & 1)) {
			//DDR_INIT_WHILE_LOOP(loop_whi_count);
			serial_puts("P_DDR1_PCTL_DFISTSTAT0 while loop\n");
		}
		writel(1, P_DDR1_PCTL_POWCTL);
		while(!(readl(P_DDR1_PCTL_POWSTAT) & 1) ) {
			//DDR_INIT_WHILE_LOOP(loop_whi_count);
			serial_puts("P_DDR1_PCTL_POWSTAT while loop\n");
		}
	}
	...
	if(ddr_channel_0){
		writel(1, P_DDR0_PCTL_SCTL);
		while(!(readl(P_DDR0_PCTL_STAT) & 1)) {
			//DDR_INIT_WHILE_LOOP(loop_whi_count);
			serial_puts("P_DDR0_PCTL_STAT while loop\n");
		}
	}

	if(ddr_channel_1){
		writel(1, P_DDR1_PCTL_SCTL);
		while(!(readl(P_DDR1_PCTL_STAT) & 1)) {
			//DDR_INIT_WHILE_LOOP(loop_whi_count);
			serial_puts("P_DDR1_PCTL_STAT while loop\n");
		}
	}
	...
	if(ddr_channel_0){
		...
		writel(3,P_DDR0_PCTL_DFITRDDATAEN);
#endif
		...
		writel((1|(3<<4)|(1<<8)|(3<<12)|(7<<16)|(1<<24)|(3<<28)), P_DDR0_PCTL_DFILPCFG0);
	...
	if(ddr_channel_1){
...
#else
		writel(2,P_DDR1_PCTL_DFITPHYWRLAT);
		writel(3,P_DDR1_PCTL_DFITRDDATAEN);
#endif
		...
		writel((1|(3<<4)|(1<<8)|(3<<12)|(7<<16)|(1<<24)|(3<<28)), P_DDR1_PCTL_DFILPCFG0);
		...
		writel((0x0 | (0x0 <<12) | (7 << 28)), P_DDR1_PUB_DTAR0);
		writel((0x08 | (0x0 <<12) | (7 << 28)), P_DDR1_PUB_DTAR1);
		writel((0x10 | (0x0 <<12) | (7 << 28)), P_DDR1_PUB_DTAR2);
		writel((0x18 | (0x0 <<12) | (7 << 28)), P_DDR1_PUB_DTAR3);
	}
	...
	if(ddr_channel_0){
		...
		while(!(readl(P_DDR0_PCTL_CMDTSTAT) & 0x1)) {
			//DDR_INIT_WHILE_LOOP(loop_whi_count);
			serial_puts("P_DDR0_PCTL_CMDTSTAT while loop\n");
		}
	}
	...
	if(ddr_channel_1){
		...
		while(!(readl(P_DDR1_PCTL_CMDTSTAT) & 0x1)) {
			//DDR_INIT_WHILE_LOOP(loop_whi_count);
			serial_puts("P_DDR1_PCTL_CMDTSTAT while loop\n");
		}
	}

	if(ddr_channel_0){
		//writel(0x05f3, P_DDR0_PUB_PIR);
		//while(!(readl(P_DDR0_PUB_PGSR0) & 0x1)) {
			////DDR_INIT_WHILE_LOOP(loop_whi_count);
			//serial_puts("P_DDR0_PUB_PGSR0 while loop\n");
		//}
		...
		serial_puts("Aml log : DDR0 - PLL,DCAL,RST,ZCAL done\n");
	}

	if(ddr_channel_1){
		//writel(0x05f3, P_DDR1_PUB_PIR);
		//while(!(readl(P_DDR1_PUB_PGSR0) & 0x1)) {
			////DDR_INIT_WHILE_LOOP(loop_whi_count);
			//serial_puts("P_DDR1_PUB_PGSR0 while loop\n");
		//}
		//PLL,DCAL,PHY RST,ZCAL
		nTempVal =	PUB_PIR_ZCAL | PUB_PIR_PLLINIT | PUB_PIR_DCAL | PUB_PIR_PHYRST;
		...
		serial_puts("Aml log : DDR1 - PLL,DCAL,RST,ZCAL done\n");
	}
	...
	if(ddr_channel_0){
		...
		serial_puts("Aml log : DDR0 - DRAM INIT done\n");
	}
	...
	if(ddr_channel_1){
		...
		serial_puts("Aml log : DDR1 - DRAM INIT done\n");
	}
	...
	if(ddr_channel_0){
		...
		serial_puts("Aml log : DDR0 - DQS done\n");
	}
	...
	if(ddr_channel_0){
		...
		serial_puts("Aml log : DDR0 - BIT deskew & data eye done\n");
	}
	...
	if(ddr_channel_1){
		...
		serial_puts("Aml log : DDR1 - BIT deskew & data eye done\n");
	}
	...
	if(ddr_channel_1){
...
#endif
	    {
			serial_puts("Aml log : DDR1 - PUB init fail with PGSR0 : 0x");
			serial_put_hex(nTempVal,32);
			serial_puts("Aml log : Try again with MMC reset...\n\n");
			DDR_INIT_ERROR_LOOP(loop_err_count);
			goto mmc_init_all;
	    ...
	    {
	        serial_puts("Aml log : DDR1 - init pass with  PGSR0 : 0x");
	...
	if(ddr_channel_0){
		writel(2, P_DDR0_PCTL_SCTL);
		while(!((readl(P_DDR0_PCTL_STAT) >> 1) & 0x1)) {
			//DDR_INIT_WHILE_LOOP(loop_whi_count);
			serial_puts("P_DDR0_PCTL_STAT while loop\n");
		}
	}
	...
	if(ddr_channel_1){
		writel(2, P_DDR1_PCTL_SCTL);
		while(!((readl(P_DDR1_PCTL_STAT) >> 1) & 0x1)) {
			//DDR_INIT_WHILE_LOOP(loop_whi_count);
			serial_puts("P_DDR1_PCTL_STAT while loop\n");
		}
	}
