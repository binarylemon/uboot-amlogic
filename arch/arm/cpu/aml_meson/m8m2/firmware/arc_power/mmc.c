...
typedef enum {
	...
	_PUB_PLLCR, _PUB_DX1GCR0, _PUB_DX2GCR0, _PUB_DX3GCR0, _PUB_DX4GCR0, _PUB_DX5GCR0,
...
void hx_ddr_power_down_enter()
{
	...
	//g_ddr_settings[M8_DDR_CHANNEL_SET]=CONFIG_M8_DDRX2_S12;
	//hx_serial_puts("g_ddr_settings[M8_DDR_CHANNEL_SET]: ");
	//hx_serial_put_hex(g_ddr_settings[M8_DDR_CHANNEL_SET],32);
	//hx_serial_puts("\n");
...
pub_init_ddr1:
	...
	{	
		//power down PLL
		//serial_puts("\nhisun log : P_DDR0_PUB_PLLCR org : 0x");
		//serial_put_hex(readl(P_DDR0_PUB_PLLCR),32);
		//writel((readl(P_DDR0_PUB_PLLCR))|0x60000000, P_DDR0_PUB_PLLCR);
		//serial_puts("\nhisun log : P_DDR0_PUB_PLLCR set : 0x");
		//serial_put_hex(readl(P_DDR0_PUB_PLLCR),32);
		//serial_puts("\n");
		//Disable DDR0 clock
	...
	if(CONFIG_M8_DDR0_ONLY == nM8_DDR_CHN_SET)
	{
		//power down PLL
		//serial_puts("\nhisun log : P_DDR0_PUB_PLLCR org : 0x");
		//serial_put_hex(readl(P_DDR1_PUB_PLLCR),32);
	...
	if(CONFIG_M8_DDR1_ONLY == nM8_DDR_CHN_SET){ //ddr channel 0 disabled, power down it.
		writel(UPCTL_CMD_SLEEP, P_DDR0_PCTL_SCTL);
	...
	if(CONFIG_M8_DDR0_ONLY == nM8_DDR_CHN_SET){ //ddr channel 1 disabled, power down it.
		writel(UPCTL_CMD_SLEEP, P_DDR1_PCTL_SCTL);
