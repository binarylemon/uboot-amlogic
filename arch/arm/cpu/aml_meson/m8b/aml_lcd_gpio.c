...
static unsigned int aml_lcd_gpio_group_ctrl[][6] = {
	...
	{P_PREG_PAD_GPIO4_EN_N, 12,      P_PREG_PAD_GPIO4_O, 12,       P_PREG_PAD_GPIO4_I, 12,}, //DIF_TTL
	{P_PREG_PAD_GPIO4_EN_N, 22,      P_PREG_PAD_GPIO4_O, 22,       P_PREG_PAD_GPIO4_I, 22,}, //HDMI_TTL
};
...
static int aml_lcd_gpio_bank_offset(int gpio, struct aml_lcd_gpio_s *gpio_s)
{
	...
	if ((gpio>=GPIOAO_0) && (gpio<=GPIOAO_13)) {
		...
		gpio_s->bank = 7;
		gpio_s->offset = gpio - DIF_TTL_0_P;
	}
	else if ((gpio>=HDMI_TTL_0_P) && (gpio<=HDMI_TTL_CK_N)) {
		gpio_s->bank = 8;
		gpio_s->offset = gpio - HDMI_TTL_0_P;
	}
	else if (gpio==GPIO_BSD_EN) {
		printf("don't support GPIO_BSD_EN Port\n");
		ret = -2;
	}
