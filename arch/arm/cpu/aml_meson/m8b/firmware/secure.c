 ...
 *			 2. 2014.07.03 v0.2 update for support 2-RSA
 *
 **************************************************************************************************************/
...
unsigned int g_action[3][16]={
	{0xd9046fb4,0xd904a3ac,0xd9040408,0xd9044358,0xd90473a0,0xd9044454,
	 0xd9044498,0xd90444e4,0xd9046d8c,0xd904ce04,0xd904cd50,0xd904a304,
	 0xd904d010,0xd9018040},
...
static int aml_m8b_sec_boot_check(unsigned char *pSRC,unsigned char *pkey1,int nkey1Len,
	 unsigned char *pkey2,int nkey2Len,unsigned int *pState)
{

#if defined(AML_SECURE_PROCESS_MSG_SHOW)
	 char * pInfo1[2][2]={
		 {"Aml log : M8B-R1024 ",   "Aml log : M8B-R2048 "},
		 {"Aml log : M8B-R1024-2X ","Aml log : M8B-R2048-2X "},
	 };
...
int aml_sec_boot_check(unsigned char *pSRC)
{
...
#if defined(AML_SECURE_PROCESS_MSG_SHOW)
	#define AML_MSG_RSA_1024 ("Aml log : M8B-R1024-1X ")
	#define AML_MSG_RSA_2048 ("Aml log : M8B-R2048-1X ")
#if defined(CONFIG_AMLROM_SPL)
	#define AML_MSG_FAIL ("TPL fail!\n")
	#define AML_MSG_PASS ("TPL pass!\n")
	#define MSG_SHOW serial_puts
	...
	nRet = aml_m8b_sec_boot_check(pSRC,0,0,0,0,&nState);
	if(nRet || !(nState & (1<<7)))
		goto exit;
	...
	nRet = aml_m8b_sec_boot_check(pSRC,pblk->sz2,((nState & (1<<30)) ? 256:128),pblk->sz3,4,0);
...
int aml_sec_boot_check_efuse(unsigned char *pSRC)
{
	...
	return aml_m8b_sec_boot_check(pSRC,sz1,sizeof(sz1),sz2,sizeof(sz2),0);
}
