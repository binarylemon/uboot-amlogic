 ...
 * @param len
 */
//void  sha256sum(sha256_hash_t *dest,uint8_t* msg,uint32_t len);
 ...
 * \param dest pointer to the location where the hash value is going to be verify  to
...
/**
 * memmove_32 - Copy one area of memory to another
 * @dest: Where to copy to
 * @src: Where to copy from
 * @count: The size of the area.
 *
 * Unlike memcpy(), memmove_32() copes with overlapping areas.
 */
static void * memmove_32(void * dest,const void *src,size_t count)
{
	char *tmp, *s;
...
/*************************************************************************/
const uint32_t sha256_init_vector[]={
 ...
 * @return none
 */
static void sha256_init(sha256_ctx_t *state){
	state->length=0;
 ...
 */
static void sha256_nextBlock (sha256_ctx_t *state, const void* block){
	...
	/* do the, fun stuff, */
		for (i=0; i<64; ++i){
			...
			memmove_32(&(a[1]), &(a[0]), 7*4); 	/* a[7]=a[6]; a[6]=a[5]; a[5]=a[4]; a[4]=a[3]; a[3]=a[2]; a[2]=a[1]; a[1]=a[0]; */
			a[4] += t1;
			a[0] = t1 + t2;
		}
 ...
 */
static void sha256_lastBlock(sha256_ctx_t *state, const void* block, uint16_t length){
	int j;
...
static void sha256_ctx2hash(sha256_hash_t *dest, const sha256_ctx_t *state){
#if defined LITTLE_ENDIAN
	uint8_t i;
	for(i=0; i<8; ++i){
...
/*************************************************************************/
static void  sha256sum(sha256_hash_t *dest,uint8_t* msg,uint32_t len)
{
	int i;
	sha256_ctx_t s;
	sha256_init(&s);
	for(i=0;i<len-(SHA256_BLOCK_BITS/8);i+=SHA256_BLOCK_BITS/8)
	{
		sha256_nextBlock(&s, &msg[i]);
	}
	sha256_lastBlock(&s, msg, SHA256_BLOCK_BITS);
	sha256_ctx2hash(dest,&s);
}
short sha_256(const sha256_hash_t * verify,  uint8_t* msg,uint32_t len)
{
	sha256_hash_t dest;
	sha256sum(&dest,msg,len);
	return (short)memcmp(dest,(*verify),sizeof(dest));
}
/*************************************************************************/
