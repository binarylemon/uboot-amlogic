...
void init_mipi_dsi_phy( void )
{
        ...
        WRITE_DSI_REG(MIPI_DSI_PHY_CTRL,  0x1 |          //enable the DSI PLL clock .
                        ...
                        (0 << 10 ) |   //select the clock freq /2.
                        (0 << 11 ) |   //select the DxDDRClk from divider.
        ...
        WRITE_DSI_REG(MIPI_DSI_LP_TIM, 0x5f3f0e0e);
                ...
                )
{
        ...
        config_video_para(output_type, color_code);
...
void mipi_dphy_init(void)
{
        ...
        // Set MIPI_PHY_CLK = VID2_PLL / 4 for 768x1024 60fps 18BPP 4lane
        WRITE_DSI_REG(MIPI_DSI_PHY_CTRL,  READ_DSI_REG(MIPI_DSI_PHY_CTRL)  &
                        ( ~(1 << 11 )));                                //select the DxDDRClk from divider.
        // Check the phylock/stopstateclklane to decide if the DPHY is ready
        check_phy_st();
...
#if 0
        switch(output_type) {
                // 240x160 is setting for 60Hz refresh rate
                case TV_ENC_LCD240x160:
                case TV_ENC_LCD240x160_dsi36b:
                        hline = MIPI_HLINE_240X160;
                        hfp   = HFP_240X160;
                        hsa   = MIPI_HSA_240X160;
                        hbp   = MIPI_HBP_240X160;
                        vsa   = VSA_240X160;
                        vbp   = VBP_240X160;
                        vfp   = VFP_240X160;
                        vact  = VACT_240X160;
                ...
                case TV_ENC_LCD240x160_dsi:
                        hline = MIPI_HLINE_240X160_dsi;
                        hfp   = HFP_240X160_dsi;
                        hsa   = MIPI_HSA_240X160_dsi;
                        hbp   = MIPI_HBP_240X160_dsi;
                        vsa   = VSA_240X160_dsi;
                        vbp   = VBP_240X160_dsi;
                        vfp   = VFP_240X160_dsi;
                        vact  = VACT_240X160_dsi;
                        break;
                case TV_ENC_LCD240x160_slow :
                        hline = MIPI_HLINE_240X160_slow;
                        hfp   = HFP_240X160_slow;
                        hsa   = MIPI_HSA_240X160_slow;
                        hbp   = MIPI_HBP_240X160_slow;
                        vsa   = VSA_240X160_slow;
                        vbp   = VBP_240X160_slow;
                        vfp   = VFP_240X160_slow;
                        vact  = VACT_240X160_slow;
                        break;
                case TV_ENC_LCD480x234:
                case TV_ENC_LCD480x234_dsi36b:
                        hline = MIPI_HLINE_480X234;
                        hfp   = HFP_480X234;
                        hsa   = MIPI_HSA_480X234;
                        hbp   = MIPI_HBP_480X234;
                        vsa   = VSA_480X234;
                        vbp   = VBP_480X234;
                        vfp   = VFP_480X234;
                        vact  = VACT_480X234;
                ...
                case TV_ENC_LCD720x480_dsi36b:
                        break;
                case TV_ENC_LCD2560x1600:
                case TV_ENC_LCD2560x1600_dsi36b:
                        hline = MIPI_HLINE_2560X1600;
                        hfp   = HFP_2560X1600;
                        hsa   = MIPI_HSA_2560X1600;
                        hbp   = MIPI_HBP_2560X1600;
                        vsa   = VSA_2560X1600;
                        vbp   = VBP_2560X1600;
                        vfp   = VFP_2560X1600;
                        vact  = VACT_2560X1600;
                ...
                case TV_ENC_LCD1920x1200p:
                        hline = MIPI_HLINE_1920X1200;
                        hfp   = HFP_1920X1200;
                        hsa   = MIPI_HSA_1920X1200;
                        hbp   = MIPI_HBP_1920X1200;
                        vsa   = VSA_1920X1200;
                        vbp   = VBP_1920X1200;
                        vfp   = VFP_1920X1200;
                        vact  = VACT_1920X1200;
                        break;
                case TV_ENC_LCD768x1024p:
#endif
                        hline = (HLINE_768X1024*9+3)/4;  // Rounded. Applicable for Period(pixclk)/Period(bytelaneclk)=9/16
                        hfp   = (HFP_768X1024*9+3)/4;
                        hsa   = (HSA_768X1024*9+3)/4;
                        hbp   = (HBP_768X1024*9+3)/4;
                        vsa   = VSA_768X1024;
                        vbp   = VBP_768X1024;
                        vfp   = VFP_768X1024;
                        vact  = VACT_768X1024;
                        ...
                        break;
                default :
                        DPRINT(" Error: Unsupport Resolution(%d), Please Add More\n", output_type);
                        break;
        }
#endif
...
void set_pll_mipi(Lcd_Config_t *p)
{
	...
	do{
        ...
        WRITE_CBUS_REG_BITS( HHI_VIID_CLK_CNTL, 0, 16, 3);//enable vid pll input, 0x104b
        WRITE_CBUS_REG_BITS( HHI_VID_CLK_DIV,   0, 0, 8 ); //0x1059
        WRITE_CBUS_REG_BITS( HHI_VID_CLK_CNTL, 1, 0, 1);   //0x105f
        WRITE_CBUS_REG_BITS( HHI_VIID_CLK_DIV,  0, 12, 4);//0x104a
        DPRINT("%s, %d\n", __func__, __LINE__);
...
void set_venc_mipi(Lcd_Config_t *pConf)
{
	...
	WRITE_LCD_REG(VPU_VIU_VENC_MUX_CTRL, 0);	//viu1, viu2 select encl

	WRITE_LCD_REG(ENCL_VIDEO_MODE,		0x40);
        WRITE_LCD_REG(ENCL_VIDEO_MODE_ADV,	0x08);// Sampling rate: 1

 	WRITE_LCD_REG(ENCL_VIDEO_FILT_CTRL,	0x1000);// bypass filter

	WRITE_LCD_REG(ENCL_VIDEO_MAX_PXCNT,	768+180-1);
	WRITE_LCD_REG(ENCL_VIDEO_MAX_LNCNT,	1024+116-1);

	WRITE_LCD_REG(ENCL_VIDEO_HAVON_BEGIN,	0+180-3);
	WRITE_LCD_REG(ENCL_VIDEO_HAVON_END,	767+180-3);
	WRITE_LCD_REG(ENCL_VIDEO_VAVON_BLINE,	0+116);
	WRITE_LCD_REG(ENCL_VIDEO_VAVON_ELINE,	1023+116);

	WRITE_LCD_REG(ENCL_VIDEO_HSO_BEGIN,	60 -1);
	WRITE_LCD_REG(ENCL_VIDEO_HSO_END,	60 + 64 -1);
	WRITE_LCD_REG(ENCL_VIDEO_VSO_BEGIN,	60 -1);
	WRITE_LCD_REG(ENCL_VIDEO_VSO_END,	60 -1 );

	WRITE_LCD_REG(ENCL_VIDEO_VSO_BLINE,	36);
	WRITE_LCD_REG(ENCL_VIDEO_VSO_ELINE,	36+50);
...
void set_control_mipi(Lcd_Config_t *p)
{
	...
	//set VPU clk
        WRITE_CBUS_REG(HHI_VPU_CLK_CNTL, 0x303); //vid pll clock, N = 0, enable
