...
//      writel(readl(P_AO_RTI_PWR_CNTL_REG0)|(1<<4),P_AO_RTI_PWR_CNTL_REG0);
	...
	writel(readl(P_AO_RTI_PWR_A9_CNTL1) & (~(1 << 1)),
	       P_AO_RTI_PWR_A9_CNTL1);

	writel(readl(P_HHI_SYS_CPU_CLK_CNTL1) | (0x1 << 30),
	       P_HHI_SYS_CPU_CLK_CNTL1);

	writel(readl(P_HHI_SYS_CPU_CLK_CNTL) | (0x7 << 16) | (0x7 << 28),
	...
	udelay__(100);
	// l2 sram sleep
	writel(readl(P_AO_RTI_PWR_A9_CNTL1) | (0x1 << 0),
	       P_AO_RTI_PWR_A9_CNTL1);
}
...
// l2 sram sleep
	...
	writel(readl(P_RESET4_REGISTER) | (1 << 0) | (1 << 3),
	       P_RESET4_REGISTER);
...
//      writel(readl(P_AO_RTI_PWR_CNTL_REG0)& (~(1<<4)),P_AO_RTI_PWR_CNTL_REG0);
//      A9_TOPCLAMP
	writel(readl(P_AO_RTI_PWR_A9_CNTL0) & (~(1 << 13)),
	       P_AO_RTI_PWR_A9_CNTL0);
...
static void switch_to_rtc()
{
	if (0x11111112 != readl(0xc11081A8))	//M8M2 do not switch ao to 32k via P_AO_RTI_PWR_CNTL_REG0
		writel(readl(P_AO_RTI_PWR_CNTL_REG0) | (1 << 8),
...
//      writel(readl(P_AO_RTI_GEN_CNTL_REG0)&(~(0x8)),P_AO_RTI_GEN_CNTL_REG0);
	...
	if (0x11111112 == readl(0xc11081A8)) {
		...
		if (wdt_flag)
			writel(readl(P_WATCHDOG_TC) & (~(1 << 19)),
			       P_WATCHDOG_TC);
	} else {
		wdt_flag = readl(P_WATCHDOG_TC) & (1 << 22);
		if (wdt_flag)
			writel(readl(P_WATCHDOG_TC) & (~(1 << 22)),
			       P_WATCHDOG_TC);
	}
#if 1
	...
	if (wdt_flag) {
		if (0x11111112 == readl(0xc11081A8))
			writel((6 * 7812 | ((1 << 16) - 1)) | (1 << 29),
			       P_WATCHDOG_TC);
		else
			writel((6 * 0x186a0 | ((1 << 22) - 1)) | (1 << 22),
			       P_WATCHDOG_TC);
	}
...
//      switch_32K_to_24M();
	...
	if (uboot_cmd_flag == 0x87654321) {
		...
		writel(readl(P_AO_RTI_PWR_CNTL_REG0) | (1 << 4),
		       P_AO_RTI_PWR_CNTL_REG0);
...
//      timer_init();
	...
	while (1) {
		...
		if (c == 't') {
			...
			init_I2C();
			while (1) {
				unsigned int run_cmd =
				    readl(P_AO_RTI_STATUS_REG0);
				int cpu;
				int state;
				if ((run_cmd >> 8) == 0x3) {
					wait_uart_empty();
					f_serial_puts
					    ("use run time firmware\n");
					wait_uart_empty();
					enter_power_down();
					writel(0, P_AO_RTI_STATUS_REG0);
					check_arm_run_state();
				...
				if ((run_cmd >> 8) == 0x1) {
					f_serial_puts("use run time cpu on\n");
					cpu = run_cmd & 0xf;
					state = (run_cmd >> 4) & 0xf;
					state = !state;
					meson_set_cpu_power_ctrl(cpu, state);
					writel(0, P_AO_RTI_STATUS_REG0);
				}
			}
			//test_arc_core();
...
void store_restore_plls(int flag)
{
	...
	if (!flag) {
		...
		writel(readl(P_HHI_SYS_PLL_CNTL) & (~(1 << 30)),
		       P_HHI_SYS_PLL_CNTL);
