...
void run_arc_program()
{
	...
	*(volatile unsigned *)(vaddr2 + 0x20) =
	    (unsigned)(&platform_reset_handler);
	...
	if (0x11111112 == m8_chip_id) {
		...
		if (((nMMC_DDR_SET >> 26) & 0x3) == 0x3) {	/*one channel */
			if (((nMMC_DDR_SET >> 24) & 0x1)) {
				address1 =
				    GET_DT_ADDR(nPUB1_DTAR0, nMMC_DDR_SET, 1);
				dbg_print("\nAml log : DDR1 DTAR is 0x",
					  address1);
			} else {
				address0 =
				    GET_DT_ADDR(nPUB0_DTAR0, nMMC_DDR_SET, 0);
		...
		case CONFIG_M8_DDR1_ONLY:
			{
				address1 =
				    CFG_M8_GET_DDR1_TA_FROM_DTAR0(nPUB1_DTAR0,
								  nMMC_DDR_SET);
				dbg_print("\nAml log : DDR1 DTAR is 0x",
					  address1);
			}
		...
		default:
			{
				address0 =
				    CFG_M8_GET_DDR0_TA_FROM_DTAR0(nPUB0_DTAR0,
								  nMMC_DDR_SET);
				address1 =
				    CFG_M8_GET_DDR1_TA_FROM_DTAR0(nPUB1_DTAR0,
								  nMMC_DDR_SET);
...
int appf_platform_enter_cstate(unsigned cpu_index, struct appf_cpu *cpu,
			       struct appf_cluster *cluster)
{
	/*
 ...
 * It is entered with cluster->lock held.
 */
int appf_platform_leave_cstate(unsigned cpu_index, struct appf_cpu *cpu,
			       struct appf_cluster *cluster)
{
	/*
	 * Housekeeping...
 ...
 * It is entered with cluster->lock held.
 */
int appf_platform_enter_cstate1(unsigned cpu_index, struct appf_cpu *cpu,
				struct appf_cluster *cluster)
{
 ...
 * It is entered with cluster->lock held.
 */
int appf_platform_leave_cstate1(unsigned cpu_index, struct appf_cpu *cpu,
				struct appf_cluster *cluster)
{
