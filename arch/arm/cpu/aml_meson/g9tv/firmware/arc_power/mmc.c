...
#ifndef __G9_DDR_SUSPEND__
#define __G9_DDR_SUSPEND__
#include <io.h>
#include "config.h"
#define readl(a) (*(volatile unsigned int *)(a))
#define writel(v,a) (*(volatile unsigned int *)(a) = (v))
//DDR0/1 channel select
...
//uboot\arch\arm\cpu\aml_meson\g9tv\firmware\ddr_init_pctl.c 
typedef enum {

	  DDR_CHANNEL_SET=0,   _MMC_DDR_CTRL, _DMC_DDR_CTRL, _SOFT_RESET , _APD_CTRL, _CLK_CTRL, // 5
...
{	
	...
	if(CONFIG_DDR1_ONLY == g_ddr_settings[DDR_CHANNEL_SET])
		nOffset = 0x2000;
...
static void hx_pctl_sleep(void)
{
	...
	if(CONFIG_DDR1_ONLY != g_ddr_settings[DDR_CHANNEL_SET]) //channel 0 used
	{
	...
	if(CONFIG_DDR0_ONLY != g_ddr_settings[DDR_CHANNEL_SET]) //channel 1 used
	{
...
{	
	...
	//does shut down PCTL clock for power saving
	//before access PCTL registers we must enable clock for PCTL
	g_ddr_settings[_DMC_DDR_CTRL] = readl(P_DMC_DDR_CTRL);
	g_ddr_settings[DDR_CHANNEL_SET] = ((((g_ddr_settings[_DMC_DDR_CTRL] >> 26)&0x3)==0x3)?(((g_ddr_settings[_DMC_DDR_CTRL]>>24)&0x1)?(CONFIG_DDR1_ONLY):(CONFIG_DDR0_ONLY)):(CONFIG_DDRX2_S12));
	...
	//if(CONFIG_DDR0_ONLY != g_ddr_settings[DDR_CHANNEL_SET])
...
#define PUB_PGSR0_DTERR     (PUB_PGSR0_RDERR|PUB_PGSR0_WDERR|PUB_PGSR0_REERR|PUB_PGSR0_WEERR)
   ...
   #undef DDR1_SUSPEND_LOAD
	...
	int nDDR_CHN_SET = g_ddr_settings[DDR_CHANNEL_SET];
...
static void hx_ddr_resume(void)
{
	...
	switch(g_ddr_settings[DDR_CHANNEL_SET])
	{
