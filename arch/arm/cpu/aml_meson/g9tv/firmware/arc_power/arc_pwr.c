...
#define M8_PLL_WAIT_FOR_LOCK(pll) \
	...
	//VID,HDMI
	//writel(readl(P_HHI_VID_PLL_CNTL5)|(1<<16), P_HHI_HDMI_PLL_CNTL5);
	//DDR
...
char *my_itou(unsigned long num, char *str)
{
...
void print_uint(const char *tag, unsigned int tmp_val)
{
#ifdef CC_ENABLE_PRINT_INT
    char tmp_buf[256];
    my_strcpy(tmp_buf, tag);
    my_itou(tmp_val, tmp_buf + my_strlen(tmp_buf));
    my_strcat(tmp_buf, "\n");
    f_serial_puts(tmp_buf);
    wait_uart_empty();
#endif
}
