...
int g9bb_pin_to_pullup(unsigned int pin ,unsigned int *reg,unsigned int *bit,unsigned int *bit_en)
{
	if(pin<=GPIOX_27)
	{
		*reg=4;
...
int g9bb_pin_map_to_direction(unsigned int pin,unsigned int *reg,unsigned int *bit)
{
	if(pin<=GPIOX_27)
	{
		*reg=4;
...
static unsigned int gpio_to_pin[][PMUX_SIZE]={
...
static int g9bb_set_pullup(unsigned pin, int val,unsigned int pullen)
{
	unsigned int reg=0,bit=0,bit_en=0,ret;
	ret=g9bb_pin_to_pullup(pin,&reg,&bit,&bit_en);
	if(!ret)
	{
		if(pullen){
			if(!ret)
...
static int g9bb_set_highz(unsigned int pin)
{
	g9bb_set_pullup((unsigned)pin,0,0);
...
struct gpio_chip amlogic_gpio_chip={
	...
	.set_pullup=g9bb_set_pullup,
	.set_highz=g9bb_set_highz,
