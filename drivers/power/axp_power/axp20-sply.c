...
static int axp_battery_adc_set(struct axp_charger *charger)
{
  ...
  | AXP20_ADC_USBVOL_ENABLE | AXP20_ADC_USBCUR_ENABLE;
	...
	ret = axp_update(AXP20_ADC_CONTROL1, val , val);

  if (ret)
    return ret;

    ret = axp_read(AXP20_ADC_CONTROL3, &val);	
  ...
  ret = axp_write(AXP20_ADC_CONTROL3, val);
  if (ret)
    return ret;
...
static void axp_charger_update_state(struct axp_charger *charger)
{
	...
	if(axp_charger == NULL)
	{
		printf("%s: Please open axp_charger first! = %d\n", __FUNCTION__, axp_charger->is_on);
		return;
	}
	axp_reads(AXP20_CHARGE_STATUS,2,val);
	tmp = (val[1] << 8 )+ val[0];
	...
	charger->is_charging = (tmp & AXP20_STATUS_INCHAR)?1:0;
...
void axp_charger_state_dump(void)
{
	if(axp_charger == NULL)
	{
		printf("%s: Please open axp_charger first! = %d\n", __FUNCTION__, axp_charger->is_on);
	...
	printf("axp_charger->is_on = %d\n",axp_charger->is_on);
	printf("axp_charger->fault = %d\n",axp_charger->fault);
	printf("axp_charger->bat_det = %d\n",axp_charger->bat_det);
	printf("axp_charger->ac_det = %d\n",axp_charger->ac_det);
	printf("axp_charger->usb_det = %d\n",axp_charger->usb_det);
	printf("axp_charger->usb_valid = %d\n",axp_charger->usb_valid);
	printf("axp_charger->ac_valid = %d\n",axp_charger->ac_valid);
	printf("axp_charger->ext_valid = %d\n",axp_charger->ext_valid);
	printf("axp_charger->bat_current_direction = %d\n",axp_charger->bat_current_direction);
	printf("axp_charger->in_short = %d\n",axp_charger->in_short);
	printf("axp_charger->batery_active = %d\n",axp_charger->batery_active);
	printf("axp_charger->low_charge_current = %d\n",axp_charger->low_charge_current);
	printf("axp_charger->is_charging = %d\n",axp_charger->is_charging);
	printf("axp_charger->int_over_temp = %d\n",axp_charger->int_over_temp);

	printf("axp_charger->vbat = %d\n",axp_charger->vbat);
	printf("axp_charger->ibat = %d\n",axp_charger->ibat);
	printf("axp_charger->vac = %d\n",axp_charger->vac);
	printf("axp_charger->iac = %d\n",axp_charger->iac);
	printf("axp_charger->vusb = %d\n",axp_charger->vusb);
	printf("axp_charger->iusb = %d\n",axp_charger->iusb);
	printf("axp_charger->ic_temp = %d\n",axp_charger->ic_temp);
...
int axp_charger_get_vbat(void)
{
    axp_charger_update(axp_charger);
	return axp_charger->vbat;
}
...
int axp_charger_set_chgcur(int chgcur)
{
    ...
    if(chgcur >= 300000 && chgcur <= 1800000){
        ...
        axp_set_bits(AXP20_CHARGE_CONTROL1, 0x80);
        axp_update(AXP20_CHARGE_CONTROL1, tmp,0x0F);
    }
    else if(chgcur == 0){
        axp_clr_bits(AXP20_CHARGE_CONTROL1, 0x80);
    }
    return 0;
}

int axp_charger_process(int bl)
{
    axp_charger_is_ac_online() ;
    if(axp_charger->ac) {
        if(bl) {
...
int axp_charger_usbvol_limit(int pmu_usbvol_limit)
{
    int var, val, tmp;
  /* usb current and voltage limit */
  if(pmu_usbvol_limit){
    axp_clr_bits(AXP20_CHARGE_VBUS, 0x01);
  	var = pmu_usbvol_limit * 1000;
    ...
    axp_set_bits(AXP20_CHARGE_VBUS, 0x03);
    
  return 0;
}

int axp_charger_usbcur_limit(int pmu_usbcur_limit)
{
  int var;
  if(pmu_usbcur_limit){
    axp_clr_bits(AXP20_CHARGE_VBUS, 0x01);
    var = pmu_usbcur_limit * 1000;
  	if(var == 900000)
    	axp_clr_bits(AXP20_CHARGE_VBUS, 0x03);
  	else if (var == 500000){
    	axp_clr_bits(AXP20_CHARGE_VBUS, 0x02);
    	axp_set_bits(AXP20_CHARGE_VBUS, 0x01);
  	}
  	else if (var == 100000){
    	axp_clr_bits(AXP20_CHARGE_VBUS, 0x01);
    	axp_set_bits(AXP20_CHARGE_VBUS, 0x02);
  	} 
  }
  else
    axp_set_bits(AXP20_CHARGE_VBUS, 0x03);
...
void axp_charger_open(void)
{
	...
	if(axp_charger == NULL)
	{
		axp_charger = malloc(sizeof(struct axp_charger));
		if(axp_charger == NULL)
		{
	...
	axp_set_bits(POWER20_IPS_SET, 0x03);
	axp_charger->sample_time = 25;
	axp_battery_adc_set(axp_charger);
	bat = axp_charger->bat_det;
	ac = is_ac();
	if((bat&&!ac)) {
	    printf("axp limit current and voltage\n");
    	axp_charger_usbvol_limit(4200); //4200mV  don't need do this ,this will cause can't boot up
    	axp_charger_usbcur_limit(900);  //900mA
	}
	else {
	    printf("axp no limit current and voltage\n");
	}
}

void axp_charger_close(void)
{
	printf("%s\n", __FUNCTION__);
	if(axp_charger)
	{
...
static void axp_set_charge(struct axp_charger *charger)
{
  ...
  uint8_t tmp=0x00;
	...
	axp_write(AXP20_CHARGE_CONTROL1,val);
	axp_update(AXP20_CHARGE_CONTROL2,tmp,0xC2);
}
#else
static void axp_set_charge(struct axp_charger *charger)
{
        ...
        void *data)
{
    ...
    axp_writes(POWER20_INTSTS1,9,w);
    ...
    struct device_attribute *attr, char *buf)
{
  ...
  uint8_t val;
  axp_read(AXP20_CHARGE_CONTROL1, &val);
  charger->chgen  = val >> 7;
    ...
    struct device_attribute *attr, char *buf)
{
  struct axp_charger *charger = dev_get_drvdata(dev);
  uint8_t val;
  axp_read(AXP20_CHARGE_CONTROL2, &val);
        ...
        struct device_attribute *attr, const char *buf, size_t count)
{
  ...
  if(var >= 40 && var <= 70){
    ...
    val &= 0x3F;
    val |= (tmp << 6);
    axp_write(AXP20_CHARGE_CONTROL2,val);
  }
  return count;
}
    ...
    struct device_attribute *attr, char *buf)
{
  struct axp_charger *charger = dev_get_drvdata(dev);
  uint8_t val;
  axp_read(AXP20_ADC_CONTROL3, &val);
  switch ((val >> 6) & 0x03){
     case 0: charger->sample_time = 25;break;
     case 1: charger->sample_time = 50;break;
     case 2: charger->sample_time = 100;break;
        ...
        struct device_attribute *attr, const char *buf, size_t count)
{
  ...
  axp_write(AXP20_ADC_CONTROL3, val);
  return count;
}
        ...
        struct device_attribute *attr, const char *buf, size_t count)
{
  ...
  var = simple_strtoul(buf, NULL, 10);
  if(var)
    axp_set_bits(AXP20_CHARGE_VBUS, 0x40);
  else
    axp_clr_bits(AXP20_CHARGE_VBUS, 0x40);
...
static void axp_earlysuspend(struct early_suspend *h)
{
    ...
    if(pmu_earlysuspend_chgcur >= 300000 && pmu_earlysuspend_chgcur <= 1800000){
    	...
    	axp_update(axp_AXP20_CHARGE_CONTROL1, tmp,0x0F);
    }
#endif
...
int Get_Bat_Coulomb_Count(struct axp_charger *charger)
{
	...
	axp_reads(AXP20_CCHAR3_RES,8,temp);
...
static void axp_charging_monitor(struct work_struct *work)
{
    ...
    if((v[0] == 0x11 || v[0] == 0x21) && charger->is_on && axp20_icharge_to_mA(charger->adc->ichar_res) > 200 && charger->vbat > 3600 && charger->disvbat != 0){
        if(((v[1] >> 7) == 0) || (((v[1] >> 3) & 0x1) == 0)){
            axp_set_bits(AXP20_CAP,0x80);
            axp_clr_bits(0xBA,0x80);            
            rdc = (axp_get_rdc(charger) * 10000 + 5371) / 10742;
            tmp = (uint16_t) rdc;
            axp_write(0xBB,tmp & 0x00FF);
            axp_update(0xBA, (tmp >> 8), 0x1F);
            axp_clr_bits(AXP20_CAP,0x80);
            axp_set_bits(0x04,0x80);    
            DBG_PSY_MSG("rdc = %d\n",rdc * 10742 / 10000);
        }
    }
    if(flag_state_change){
		...
		if((Cap_Index2 >= 2) && (charger->disvbat != 0)){
			charger->disvbat = 0;
			charger->disibat = 0;
			Cap_Index2 = 0;
		}
    ...
    if((charger->rest_vol - pre_rest_vol)||(prev_acin != charger->ext_valid)){
        ...
        pre_rest_vol = charger->rest_vol;
        axp_write(AXP20_DATA_BUFFER1,charger->rest_vol | 0x80);
        if(charger->rest_vol == 100){
        	axp_clr_bits(0x32,0x38);
        }
        power_supply_changed(&charger->batt);
...
static int axp_battery_probe(struct platform_device *pdev)
{
  ...
  ret = axp_register_notifier(&charger->nb, AXP20_NOTIFIER_ON);
  if (ret)
    goto err_notifier;
...
//	if(!((val >> 1) & 0x01))
  ...
  /* 3.5552V--%5 close*/
  axp_write(AXP20_APS_WARNING1,0x7A);
  ...
  /* n_oe delay time set */
	...
	axp_read(POWER20_OFF_CTL,&val);
	val &= 0xfc;
	val |= ((pmu_pwrnoe_time) / 1000);
	axp_write(POWER20_OFF_CTL,val);
	DBG_PSY_MSG("%d-->0x%x\n",__LINE__,val);
	
	/* pek open time set */
	axp_read(POWER20_PEK_SET,&val);
	...
	if(pmu_intotp_en)
		pmu_intotp_en = 1;
	axp_read(POWER20_HOTOVER_CTL,&val);
	val &= 0xfb;
	val |= pmu_intotp_en << 2;
	axp_write(POWER20_HOTOVER_CTL,val);
	DBG_PSY_MSG("%d-->0x%x\n",__LINE__,val);

	axp_charger_update_state(charger);

  axp_read(AXP20_DATA_BUFFER1,&val1);
  charger->rest_vol = (int) (val1 & 0x7F);

  axp_read(AXP20_CAP,&val2);
  ...
  Total_Cap = charger->rest_vol * AXP20_VOL_MAX;
  /*enable coulomb*/
  axp_set_bits(0xB8,0x80);
  /* disable */
  axp_set_bits(AXP20_CAP,0x80);
  axp_clr_bits(0xBA,0x80);
  axp_read(AXP20_DATA_BUFFER0,&val1);
  if(((val1 >> 7) & 0x1) == 0){
  	rdc = (pmu_battery_rdc * 10000 + 5371) / 10742;
  	tmp2 = (uint16_t) rdc;
  	axp_write(0xBB,tmp2 & 0x00FF);
  	axp_update(0xBA, (tmp2 >> 8), 0x1F);
  }
	axp_clr_bits(AXP20_CAP,0x80);
...
err_ps_register:
  axp_unregister_notifier(&charger->nb, AXP20_NOTIFIER_ON);
...
static int axp20_suspend(struct platform_device *dev, pm_message_t state)
{
    ...
    irq_w[8] = 0xff;
    axp_writes(POWER20_INTSTS1, 9, irq_w);
    ...
    axp_update(AXP20_CHARGE_CONTROL1, tmp,0x0F);
		...
		tmp = 0xff;
		axp_read(POWER20_COULOMB_CTL, &tmp);

		//tmp |= 0x20;
		tmp &= 0x3f;

		axp_write(POWER20_COULOMB_CTL, tmp);
		tmp |= 0x80;
		tmp &= 0xbf;
...
static int axp20_resume(struct platform_device *dev)
{
    ...
    axp_register_notifier(&charger->nb, AXP20_NOTIFIER_ON);
