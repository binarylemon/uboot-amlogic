...
#define CONFIG_EXTRA_ENV_SETTINGS \
	...
	"upgrade_check=if itest ${upgrade_step} == 0; then defenv; save; run update; else if itest ${upgrade_step} == 1; then if test ${reboot_mode} = usb_burning; then defenv_without reboot_mode; setenv upgrade_step 2; save; fi; fi\0" \
