...
#if defined(WRITE_TO_NAND_ENABLE) && defined(CONFIG_SECURITYKEY)
#define CONFIG_AML_NAND_KEY
#endif
#if defined(WRITE_TO_EMMC_ENABLE) && defined(CONFIG_SECURITYKEY)
...
#define CONFIG_EXTRA_ENV_SETTINGS \
	...
	"upgrade_check=if itest ${upgrade_step} != 2; then defenv_without reboot_mode;setenv upgrade_step 2; save; mmc read 1 82000000 0 1; mw.w 820001fe 0; mmc write 1 82000000 0 1; fi\0" \
	...
	"nandargs=run cvbscheck; setenv bootargs init=/init console=${console} androidboot.resolution=${outputtemp} hdmimode=${hdmimode} cvbsmode=${cvbsmode} hlt vmalloc=256m mem=1024m a9_clk_max=1512000000\0"\
	...
	"nandboot=echo Booting from eMMC ...;run nandargs;get_efuse_adc;mmc read 1 ${loadaddr} a000 3000;bootm;run recovery\0" \
