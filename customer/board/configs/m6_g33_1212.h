...
#define CONFIG_EXTRA_ENV_SETTINGS \
	...
	"preboot=if itest ${upgrade_step} == 1; then defenv; setenv upgrade_step 2; save; fi; run val_init; nand read logo ${loadaddr_misc} 0 1000000; unpackimg ${loadaddr_misc};usbbc; chk_all_regulators; get_rebootmode; clear_rebootmode; echo reboot_mode=${reboot_mode}; if test ${reboot_mode} = usb_burning; then run usb_burning; fi; run upgrade_check; run batlow_or_not; saradc open 4; run updatekey_or_not; run usb_burning_or_not; run switch_bootmode\0" \
	...
	"recovery=run prepare; bmp display ${bootup_offset}; if mmcinfo; then if fatload mmc 0 ${loadaddr} recovery.img; then run start_recovery;fi;fi;  if nand read recovery ${loadaddr} 0 600000; then run start_recovery; else echo no uImage_recovery in NAND; fi\0" \
	...
	"batlow_or_not=get_batcap; "                                            \
        "if usb_online_only; then "                                         \
            "echo usb_online_only; "                                        \
            "if itest ${battery_cap} < ${power_up_threshold}; then "        \
                "echo battery_cap: ${battery_cap}, goto suspend; "          \
                "suspend early; "                                           \
            "fi; "                                                          \
        "fi; "                                                              \
        "if ac_online; then "                                               \
            "; "                      /* nothing to do */                   \
        "else "                                                             \
            "if itest ${battery_cap} < ${batlow_threshold}; then "          \
                "run prepare; "                                             \
                "run batlow_warning; "                                      \
                "poweroff; "                                                \
            "fi; "                                                          \
	...
	"start_recovery=setenv bootargs ${bootargs} a9_clk_max=800000000; bootm; \0"


#define CONFIG_BOOTCOMMAND  "bmp display ${bootup_offset}; nand read boot ${loadaddr} 0 600000; setenv bootargs ${bootargs} a9_clk_max=1512000000; bootm"
